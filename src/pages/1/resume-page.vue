        ...
        <div v-for="(detail, i) in json.details" :key="`detail${i}`">
          ...
          <div class="q-pt-md q-pb-lg">
            ...
            >
              ...
              <div
                class="text-right"
                v-else-if="article.rating || article.progress"
              >
                <template v-if="article.type == 'progress'">
                  <q-linear-progress
                    padding="0"
                    size="lg"
                ...
                <template v-else-if="article.type == 'rating'">
                  <q-rating
                    readonly
                ...
                <div class="text-italic" v-html="ratings[article.rating - 1]" />
              </div>
            </div>
          </div>
        </div>
...
import(`../data/${id}.json`).then((module) => {
  ...
  (
    data.details as { articles: { rating: number; progress: number }[] }[]
  ).forEach((detail) => {
    detail.articles.forEach((article) => {
      article.rating = Math.round(article.progress / 20);
    });
  });
